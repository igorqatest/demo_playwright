...
test('Shopping only Primary', async ({ page, context }) => {
  ...
  await page.goto('https://brella--qa.sandbox.lightning.force.com/lightning/r/Contract/800QL00000JoTVJYA3/view');
...
// Select Ansel benefits
  await newPage.getByRole('button', { name: 'Next step' }).click();
    ...
    try {
        const spouseButton = await newPage.getByText('My spouse and me');
        await spouseButton.click({ force: true, timeout: 3000 });
        console.log('🟢 Clicked "My spouse and me"');
      } catch (error) {
        console.log('⚪ "My spouse and me" not found or already moved on');
      }
