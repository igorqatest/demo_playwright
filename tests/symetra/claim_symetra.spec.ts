...
test('My first test', async ({ page }) => {
  ...
  await page.goto(config.urlMemberSymetra, { waitUntil: 'domcontentloaded' });

  console.log('✅ Filling in Login form...');
  const emailInput = page.getByRole('textbox', { name: 'Enter your email' });
  await emailInput.type(config.userMemberSymetra, { delay: 10 });

  const passwordInput = page.locator('input[name="password"]');
  await passwordInput.type(config.passwordMemberSymetra, { delay: 10 });
